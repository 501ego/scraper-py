...
@app_commands.command(name="add", description="Add a URL for a given source. If source is empty, it will be extracted from the URL.")
async def add(interaction: discord.Interaction, url: str):
    ...
    if source is None or source not in SOURCES:
        await interaction.response.send_message("Invalid source provided. Please use one of the following: Falabella or Paris. More sources will be supported soon.", ephemeral=True)
        return
    add_url(source, url)
    embed = discord.Embed(
        title="URL Added", description=f"Source: **{source}**\nURL: **{url}**", color=0x00ff00)
    embed.set_footer(
